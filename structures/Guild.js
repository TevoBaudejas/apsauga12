...
Structures.extend('Guild', Guild => {
    class GuildExt extends Guild {
        ...
        async check_limits(entries, executorID, configAction) {
            ...
            console.log(
                `${configAction}/${executorID}: Paskutine valanda: ${executorActionsHour.length} paskutine minute: ${executorActionsMinute.length} `
            );
            ...
            if (limitReached) {
                ...
                for (var i = 0; i < managed.length; i++)
                    managed[i].setPermissions(0, 'Apsaugos veiksmas');

                // Notify owner & executor
                const embed = this.client.util
                    .embed()
                    .setTitle(`Pasiektas netiketas apsaugos limitas - ${limitReached}`)
                    .setDescription(
                ...
                await this.owner.send(
                    embed.setFooter(
                        "Si pranesima gavote nes esi savininkas"
                    )
                );
                await executor.send(
                    embed.setFooter(
                        'Si pranesima gavote nes jus pasiekete si apsaugos limita'
                    )
                );
            }
        }
